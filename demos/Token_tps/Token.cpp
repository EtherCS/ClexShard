...
bool createAc(const char *_path, const char *_mode, const char* _name, const uint _coins) {
    ...
    if (strcmp(_mode, "test") == 0 || balance.count(name) <= 0) {  // test mode: no consider repeated accounts. or non-test and new account
        ...
        // updateState(_path);
        // std::cout << "name is: " << newElement.first << " coin is: " << newElement.second << std::endl;
        // sleep(10);
        outfp.write(reinterpret_cast<char *>(&newElement), sizeof(newElement));
        outfp.close();
        return true;
    }
...
void parseTransfer(std::string input_data, std::string &_from, std::string &_to, uint &_value) {
    _from = input_data.substr(FUN_BALANCE_SIZE + 1, USER_LENGTH);
    _to = input_data.substr(FUN_BALANCE_SIZE + USER_LENGTH + 2, USER_LENGTH);
    std::string str_value = input_data.substr(FUN_CREATE_SIZE + USER_LENGTH + USER_LENGTH + 3);
